...
def scrap_product_reviews(url):
    source=requests.get(url).text
    #parse the source
    soup=BeautifulSoup(source,"lxml")
    review_ps=soup.find_all('p',class_='wt-text-truncate--multi-line') #class name of all the review elements
    reviews=[]

    for rev in review_ps:
